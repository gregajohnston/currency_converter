...
class CurrencyConverter:
    ...
    def convert(self, from_currency, to_code):
        if (to_code not in self.exchange_rate.keys() or
            from_currency.code not in self.exchange_rate.keys()):
            raise UnknownCurrencyCodeError("Currency is missing. Cannot convert.")
        elif from_currency.code == to_code:
            to_currency = from_currency
            return to_currency
        else:
            to_currency = Currency(to_code, from_currency.value * self.exchange_rate[to_code] / self.exchange_rate[from_currency.code])
            return to_currency
